...
class PlacePostViewController: UIViewController, UITableViewDelegate, UITableViewDataSource {

    @IBOutlet weak var placeposttableview: UITableView!
    ...
    override func viewDidLoad() {
        ...
        Alamofire.request("http://35.226.10.223/ios.php", method: .get, parameters: para).validate().responseJSON { response in
            ...
            self.placeposttableview.reloadData()
            
        }
        
        //auto resizing row height
        self.placeposttableview.estimatedRowHeight = 140.0
        self.placeposttableview.rowHeight = UITableViewAutomaticDimension
        
        //avoid extra empty row
        self.placeposttableview.tableFooterView = UIView(frame: CGRect.zero)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = self.placeposttableview.dequeueReusableCell(withIdentifier: "placepostcell", for: indexPath) as! PlacePostTableViewCell
        
        let url = URL(string: self.imgurl)
        DispatchQueue.global().async {
            let data = try? Data(contentsOf: url!)
            DispatchQueue.main.async {
                cell.placepostimg.image = UIImage(data: data!)
            }
        }
        ...
        if message != nil {
            cell.placepostcontent.text = array[indexPath.row]["message"] as? String
        }
        else {
            cell.placepostcontent.text = "No data found"
        }
        ...
        //get post date
        cell.placepostdate.text = dateString
