...
class EventAlbumViewController: UIViewController, UITableViewDataSource,UITableViewDelegate {

    @IBOutlet weak var eventalbumtableview: UITableView!
    var id = String()
    var array = [AnyObject]()
    ...
    override func viewDidLoad() {
        ...
        Alamofire.request("http://35.226.10.223/ios.php", method: .get, parameters: para).validate().responseJSON { response in
            ...
            if result.value != nil {
                ...
                self.eventalbumtableview.reloadData()
                
                }
        }
        
        self.eventalbumtableview.tableFooterView = UIView(frame: CGRect.zero)
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = self.eventalbumtableview.dequeueReusableCell(withIdentifier: "eventalbumcell", for: indexPath) as! EventAlbumTableViewCell
        cell.eventalbumname.text = array[indexPath.row]["name"] as! String
            ...
            if let picpd = pic?["data"] as? [[String:Any]] {
                ...
                DispatchQueue.global().async {
                    ...
                    DispatchQueue.main.async {
                        cell.eventalbumpicone.image = UIImage(data: data!)
                    }
                }
                ...
                if i+1 == arraypic.count {
                    ...
                    DispatchQueue.global().async {
                        ...
                        DispatchQueue.main.async {
                            cell.eventalbumpictwo.image = UIImage(data:datatwo!)
                        }
                    }
                }
            }
    ...
    func tableView(_ tableView: UITableView, willDisplay cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        (cell as! EventAlbumTableViewCell).watchFrameChanges()
    }
    func tableView(_ tableView: UITableView, didEndDisplaying cell: UITableViewCell, forRowAt indexPath: IndexPath) {
        (cell as! EventAlbumTableViewCell).ignoreFrameChanges()
    }
    
    
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        if indexPath as NSIndexPath == selectedIndexPath {
            return EventAlbumTableViewCell.expandedHeight
        }
        else {
            return EventAlbumTableViewCell.defaultHeight
        }
    }
